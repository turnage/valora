...
impl Sketch for Pendulum {
    fn sketch(&self, _ctx: &SketchContext, mut rng: StdRng) -> Result<Composition> {
        ...
        let splotch = Ngon { n: 5, center: Point::center(), rotation: 0.0, radius: 0.1 }
            .as_irregular();
        let splotch = spawn(&WaterColor::new(splotch,
                                             WaterColorCfg {
        ...
        let n = 30;
        let offset = 10;
        let pendulum = spawn(&Instancer::with((), move |_, cfg| {
            ...
            Tween::from(Mesh {
                    ...
                    .anim_scale(0.0,
                                1.,
                                Interpolation::Linear { start: cfg.index * offset, len: 100 })
        }),
                             &vec![Point::center()]
                                  .into_iter()
                                  .cycle()
        ...
        Ok(Composition::new()
               .solid_layer(Colorer::from(Colora::hsv(RgbHue::from(49.0), 0.2, 0.0, 1.0)))
               .add(pendulum)
               .add(splotch)
               .add(Mesh {
                        src: Rect::frame(),
...
fn main() {
    sketch(SketchCfg { size: 1080, root_frame_filename: Some(String::from("jungle")), seed: None },
           Pendulum {})
            .expect("sketch");
}
